...
export default function Home() {
  return (
    <div className="max-w-7xl mx-auto w-full">
      <h1 className="text-3xl font-bold mb-8 text-[#2D3748]">ArchiDAO Projeleri</h1>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8">
        {projects.map((project) => (
          <div key={project.id} className="bg-white rounded-lg shadow p-4 flex flex-col items-center">
            <img src={project.image} alt={project.name} className="w-full h-40 object-cover rounded mb-4" />
            <div className="font-semibold text-lg mb-1 text-[#2D3748]">{project.name}</div>
            <div className="text-sm text-gray-500 mb-2">Mimar: {project.architect}</div>
